...
func FriendList(userId uint) (*[]models.UserBasic, error) {
	...
	// SELECT * FROM `relations` WHERE owner_id = userId and type=1
...
func AddFriend(userID, TargetId uint) (int, error) {
	...
	// SELECT * FROM `relations` WHERE owner_id = userID and target_id = TargetId and type = 1
	...
	// SELECT * FROM `relations` WHERE owner_id = TargetId and target_id = userID and type = 1
